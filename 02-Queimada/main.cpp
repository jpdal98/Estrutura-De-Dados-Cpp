...
int queimar(vector<string> &mat, int l, int c, int nivel){
    ...
    if((mat[l][c] != '#') )
        return 0;
    if( nivel > 9)//controle pra nivel sempre ser  < 10
        nivel = 0;
	...
	for(Par p : embaralhar(getNeib(Par(l, c)))){
			contador +=  queimar(mat, p.l, p.c, nivel);//nâo é necessário o nivel +1, pois ao fazer a recursao as instruções de nivel acontecem novamente.
